...
export function analizarTrigger(
  ...
  if (bufferEventos.length === 0) {
    return trigger;
  }

  if (contieneRueda(bufferEventos)) {
    return analizarRueda(bufferEventos);
  }

  const ultimoDown = buscarUltimoDown(bufferEventos);

  if (!ultimoDown) {
    return trigger;
  }

  const codigo = ultimoDown.entrada.codigo;

  const bloque = bufferEventos.filter(
    (evento) => evento.entrada.codigo === codigo,
  );

  trigger.gatillo = ultimoDown.entrada;

  analizarCondicion(trigger, bloque);

  trigger.modificadores = limpiarModificadores(bufferEventos, codigo);

  normalizarAltGr(trigger);
  ...
  if (!permitirClickIzquierdo && esClickIzquierdoSolo(trigger)) {
    return crearTrigger();
  }
...
function contieneRueda(bufferEventos: EventoBuffer[]): boolean {
...
function analizarRueda(bufferEventos: EventoBuffer[]): Trigger {
  const trigger = crearTrigger();

  const ruedas = bufferEventos.filter(
    (evento) =>
      evento.entrada.codigo === "WheelUp" ||
      evento.entrada.codigo === "WheelDown",
  );

  const arriba = ruedas.filter((evento) => evento.entrada.codigo === "WheelUp");

  const abajo = ruedas.filter(
    (evento) => evento.entrada.codigo === "WheelDown",
  );

  const entradaRueda = arriba.length >= abajo.length ? arriba[0] : abajo[0];
...
function obtenerModificadoresRueda(bufferEventos: EventoBuffer[]) {
  const usados = new Set<string>();

  const resultado: (typeof bufferEventos)[number]["entrada"][] = [];
  ...
  for (const evento of bufferEventos) {
    ...
    if (evento.evento !== "Down") {
      continue;
    }

    if (usados.has(evento.entrada.codigo)) {
      continue;
    }

    usados.add(evento.entrada.codigo);

    resultado.push(evento.entrada);
  }
...
function buscarUltimoDown(
  ...
  for (let i = bufferEventos.length - 1; i >= 0; i--) {
    const evento = bufferEventos[i];

    if (evento.evento !== "Down") {
      continue;
    }

    const tieneUp = bufferEventos.some(
      (siguiente) =>
        siguiente.entrada.codigo === evento.entrada.codigo &&
        siguiente.evento === "Up" &&
...
function analizarCondicion(trigger: Trigger, bloque: EventoBuffer[]): void {
  ...
  const primerDown = bloque.find((evento) => evento.evento === "Down");
...
function limpiarModificadores(
  ...
  for (const evento of bufferEventos) {
    ...
    if (evento.entrada.codigo === codigoGatillo) {
      continue;
    }
