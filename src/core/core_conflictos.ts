...
function triggersIguales(
  ...
  if (triggerA.modificadores.length !== triggerB.modificadores.length) {
    return false;
  }

  for (let indice = 0; indice < triggerA.modificadores.length; indice++) {
    const entradaA = triggerA.modificadores[indice];

    const entradaB = triggerB.modificadores[indice];
